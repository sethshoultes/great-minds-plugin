...
function startPrdWatcher(): void {
  ...
  const handleNewOrChanged = (filePath: string): void => {
    ...
    // ── PRD lint pre-flight ──────────────────────────────────
    const lintScript = resolve(PRDS_DIR, "..", "pipeline", "auto", "prd-lint.sh");
    if (existsSync(lintScript)) {
      try {
        execSync(`"${lintScript}" "${filePath}"`, { encoding: "utf-8", timeout: 10_000 });
        log(`WATCHER: Lint PASS for "${name}"`);
      } catch (err: any) {
        const lintFailedDir = resolve(PRDS_DIR, "lint-failed");
        try { execSync(`mkdir -p "${lintFailedDir}"`); } catch {}
        try {
          execSync(`mv "${filePath}" "${lintFailedDir}/${name}"`);
          const reportSrc = filePath.replace(/\.md$/, ".lint-report.md");
          const reportDst = resolve(lintFailedDir, name.replace(/\.md$/, ".lint-report.md"));
          if (existsSync(reportSrc)) execSync(`mv "${reportSrc}" "${reportDst}"`);
          log(`WATCHER: Lint FAIL for "${name}" — moved to prds/lint-failed/. See ${reportDst}`);
        } catch (mvErr) {
          log(`WATCHER: Lint FAIL for "${name}" but move failed: ${mvErr}`);
        }
        return;
      }
    }
  ...
  watcher.on("add", handleNewOrChanged);
  watcher.on("change", handleNewOrChanged);
