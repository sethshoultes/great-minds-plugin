...
import {
  ...
  MEMORY_STORE_DIR, REPO_PATH, PRDS_DIR, DELIVERABLES_DIR, INTAKE_PRIORITY_LABELS,
} from "./config.js";
  ...
  for (const repo of GIT_REPOS) {
    try {
      ...
      if (dirty) {
        ...
        // Protect daemon source files from agent-induced edits.
        // If protected files are dirty, log loudly so we can audit before commit.
        if (repo.name === "great-minds-plugin") {
          const protectedPaths = [
            "daemon/src/pipeline.ts",
            "daemon/src/daemon.ts",
            "daemon/src/agents.ts",
            "daemon/src/config.ts",
            "daemon/src/health.ts",
            "daemon/src/cost-tracker.ts",
          ];
          for (const path of protectedPaths) {
            try {
              const diff = execSync(
                `git -C "${repo.path}" diff --shortstat -- "${path}" 2>/dev/null`,
                { encoding: "utf-8", timeout: 5_000 },
              ).trim();
              if (diff) log(`GIT: PROTECTED-DIRTY ${repo.name}/${path} — ${diff}`);
            } catch {}
          }
        }
