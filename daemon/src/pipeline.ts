...
// Build-phase model override (env BUILD_PHASE_MODEL). Default "sonnet" maps to Kimi via Ollama. Set BUILD_PHASE_MODEL=qwen3.5:cloud to test alternate model for build phase only.
const BUILD_MODEL = process.env.BUILD_PHASE_MODEL || "sonnet";
...
async function runAgentCore(name: string, prompt: string, maxTurns = DEFAULT_MAX_TURNS, phase = "", model?: string): Promise<string> {
  ...
  try {
    ...
    try {
      logCost({
        prdId: currentProject || name,
        model: model || 'default',
        tokensIn: inputTokens,
        tokensOut: outputTokens,
        costUsd: estimateCost(inputTokens, outputTokens),
        durationMs: Math.round(parseFloat(elapsed) * 1000) || 0,
        timestamp: new Date().toISOString(),
        phase: (phase || name.split('-')[0]) as any,
        success: !result.includes('ERROR') && !result.includes('FAIL'),
      });
    } catch (e) {
      log(`COST WARN: ${e}`);
    }
...
Put all output in ${delDir}/. Write ${resolve(REPO_PATH, ".planning/execution-report.md")} when done.`;
  ...
  await runAgent("builder", builderPrompt,
    isHotfix ? 15 : DEFAULT_MAX_TURNS, "build", BUILD_MODEL);
...
Commit fixes.`, DEFAULT_MAX_TURNS, "build", BUILD_MODEL);
      }
    }
  }
